...
router.post("/", async (req, res) => {
  try {
    ...
    await db.query(
      `INSERT INTO vending_machines (vm_id, vm_name, org_id)
       VALUES (?, ?, ?)`,
      [vm_id, vm_name, org_id]
    );
    res.status(200).json({ vm_id, vm_name, org_id });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
router.patch("/:id/mode", async (req, res) => {
  try {
    ...
    if (!["i", "r", "t"].includes(vm_mode)) {
      return res.status(400).json({ error: "Invalid vending machine mode" });
    }
    const [results] = await db.query(
      "UPDATE vending_machines SET vm_mode = ? WHERE vm_id = ?",
      [vm_mode, req.params.id]
    );
