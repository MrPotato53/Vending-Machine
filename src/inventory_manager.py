...
class InventoryManager:
    ...
    def set_mode(self, new_mode: Mode) -> None:
        if(new_mode is Mode.IDLE and self.mode is Mode.IDLE):
                raise ValueError("Cannot change mode from IDLE to IDLE")

        if(new_mode is Mode.TRANSACTION and self.mode is not Mode.IDLE):
            raise ValueError("Mode must be IDLE before changing to TRANSACTION, not" \
                             + Mode.IDLE.name)

        if(new_mode is Mode.RESTOCKING and self.mode is not Mode.IDLE):
    ...
    def change_stock(self, slot_name: str, item_stock: int) -> float:
        ...
        item: Item = self.items[itemrow][itemcol]
